...
class AriaMotionSensorPlayer : public RecordFormatStreamPlayer {
  ...
  bool onDataLayoutRead(const CurrentRecord &r, size_t blockIndex,
                        DataLayout &dl) override {
    if (r.recordType == Record::Type::CONFIGURATION) {
      auto &config = getExpectedLayout<aria::MotionSensorConfigRecordMetadata>(
          dl, blockIndex);
      buffer.clear();
      ...
      auto &data = getExpectedLayout<aria::MotionSensorDataRecordMetadata>(
          dl, blockIndex);
      buffer.clear();
      fmt::format_to(back_inserter(buffer), "{},",
                     data.captureTimestampNs.get());
      if (hasAcc) {
        vector<float> accData;
        data.accelMSec2.get(accData);
        fmt::format_to(back_inserter(buffer), "{},{},{},", accData[0],
                       accData[1], accData[2]);
      }
      if (hasGyro) {
        vector<float> gyroData;
        data.gyroRadSec.get(gyroData);
        fmt::format_to(back_inserter(buffer), "{},{},{},", gyroData[0],
                       gyroData[1], gyroData[2]);
      }
      if (hasMag) {
        vector<float> magData;
        data.magTesla.get(magData);
        fmt::format_to(back_inserter(buffer), "{},{},{},", magData[0],
                       magData[1], magData[2]);
      }
      line = to_string(buffer);
      line.pop_back();
...
public:
  AriaMotionSensorPlayer(string filename)
      : RecordFormatStreamPlayer(), csvFile(), hasAcc(false), hasGyro(false),
        hasMag(false), buffer(), line() {
...
class AriaWifiBeaconPlayer : public RecordFormatStreamPlayer {
  ...
  bool onDataLayoutRead(const CurrentRecord &r, size_t blockIndex,
    ...
    if (r.recordType == Record::Type::CONFIGURATION) {
      ...
      auto &data =
          getExpectedLayout<aria::WifiBeaconDataRecordMetadata>(dl, blockIndex);
      buffer.clear();
      fmt::format_to(back_inserter(buffer), "{},{},{},{},{}\n",
                     data.boardTimestampNs.get(), data.ssid.get(),
                     data.bssidMac.get(), data.freqMhz.get(), data.rssi.get());
...
public:
  AriaWifiBeaconPlayer(string filename)
      : RecordFormatStreamPlayer(), csvFile(), buffer() {
    csvFile.open(filename);
  }
};
...
class AriaBluetoothBeaconPlayer : public RecordFormatStreamPlayer {
  ...
  bool onDataLayoutRead(const CurrentRecord &r, size_t blockIndex,
    ...
    if (r.recordType == Record::Type::CONFIGURATION) {
      ...
      auto &data = getExpectedLayout<aria::BluetoothBeaconDataRecordMetadata>(
          dl, blockIndex);
      buffer.clear();
      fmt::format_to(back_inserter(buffer), "{},{},{},{}\n",
                     data.boardTimestampNs.get(), data.uniqueId.get(),
                     data.rssi.get(), data.freqMhz.get());
      csvFile << to_string(buffer);
    }
    return true;
  }
...
public:
  AriaBluetoothBeaconPlayer(string filename)
      : RecordFormatStreamPlayer(), csvFile(), buffer() {
    csvFile.open(filename);
  }
};
...
class AriaBarometerPlayer : public RecordFormatStreamPlayer {
  ...
  bool onDataLayoutRead(const CurrentRecord &r, size_t blockIndex,
    ...
    if (r.recordType == Record::Type::CONFIGURATION) {
      ...
      auto &data =
          getExpectedLayout<aria::BarometerDataRecordMetadata>(dl, blockIndex);
      buffer.clear();
      fmt::format_to(back_inserter(buffer), "{},{},{}\n",
                     data.captureTimestampNs.get(), data.pressure.get(),
                     data.altitude.get());
      csvFile << to_string(buffer);
    }
    return true;
...
public:
  AriaBarometerPlayer(string filename)
      : RecordFormatStreamPlayer(), csvFile(), buffer() {
    csvFile.open(filename);
  }
};
...
class AriaTimeSyncPlayer : public RecordFormatStreamPlayer {
  ...
  bool onDataLayoutRead(const CurrentRecord &r, size_t blockIndex,
                        DataLayout &dl) override {
    if (r.recordType == Record::Type::CONFIGURATION) {
      csvFile << "timestamp,realtime\n";
    } else if (r.recordType == Record::Type::DATA) {
      auto &data =
          getExpectedLayout<aria::TimeSyncDataRecordMetadata>(dl, blockIndex);
      buffer.clear();
      fmt::format_to(back_inserter(buffer), "{},{}\n",
                     data.monotonicTimestampNs.get(),
                     data.realTimestampNs.get());
      csvFile << to_string(buffer);
    }
    return true;
  }
...
public:
  AriaTimeSyncPlayer(string filename)
      : RecordFormatStreamPlayer(), csvFile(), buffer() {
    csvFile.open(filename);
  }
};
...
struct VRSReader {
  ...
  static void readFile(const string &vrsFilePath) {
    ...
    string exp = vrsFilePath.substr(0, vrsFilePath.length() -
                                           4); // remove .vrs extension
      ...
      for (auto id : reader.getStreams()) {
        ...
        case RecordableTypeId::TimeRecordableClass:
          streamPlayer = make_unique<AriaTimeSyncPlayer>(
              fmt::format("{}_Time_{}.csv", exp, id.getInstanceId()));
          break;
        default:
          break;
        }
